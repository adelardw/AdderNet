...
class GiGaSpectrogramLightningModelUnet(L.LightningModule):
    ...
    def get_mag_phase(self, mixed_waveforms):
        ...
        stft_mixed = self.stft(mixed_waveforms)
        
        phase = torch.angle(stft_mixed)
    ...
    def _step(self, batch, kind):
        ...
        stft_clean = self.stft(speech_waveforms)
        stft_mixed = self.stft(mixed_waveforms)
        
        clean_magnitude = torch.abs(stft_clean)
        clean_phase = torch.angle(stft_clean)
        
        mixed_magnitude = torch.abs(stft_mixed)
        mixed_phase = torch.angle(stft_mixed)
...
class UltraSpectrogramLightningModelUnet(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.Adam([
            {'params': self.model.parameters()},
            {'params': self.phase_model.parameters(), 'lr': 1e-4},  
            {'params': [self.alpha, self.beta, self.gamma], 'lr': 1e-3}         
        ], lr=3.41e-4,weight_decay=0.2)
...
class UltraMaxSpectrogramLightningModelUnet(L.LightningModule):
    def __init__(self, encoder_parameters,decoder_parameters, stft_attributes, audio_encoder_parameters,
                 audio_len = 128000):
        super().__init__()
        self.save_hyperparameters()
        ...
        self.stft = torchaudio.transforms.Spectrogram(**stft_attributes)
        self.model = DenoisingModelUnetBottleneckWithAudioEncoder(encoder_parameters=encoder_parameters,
                                                                  decoder_parameters=decoder_parameters,
                                                                  audio_encoder_parameters=audio_encoder_parameters)
    ...
    def get_mag_phase(self, mixed_waveforms):
        ...
        with torch.no_grad():
            output_magnitude = self.model(magnitude, mixed_waveforms)
            mag_padded = self.pad_or_trim(output_magnitude, magnitude)
            output_phase =  self.phase_model(mag_padded, phase)
            phase_padded = self.pad_or_trim(output_phase, phase)
    ...
    def _step(self, batch, kind):
        ...
        output_magnitude = self.model(mixed_magnitude, mixed_waveforms)
        output_magnitude = self.pad_or_trim(output_magnitude, stft_mixed)
        output_phase =  self.phase_model(output_magnitude, mixed_phase)
        ...
        triplet_loss = self.triplet_loss(stft_clean, stft_pred, stft_mixed)
        loss = reconstruct_loss  + phase_loss + psl_loss + spectral_loss + 0.1*stft_loss + 0.1*triplet_loss 
